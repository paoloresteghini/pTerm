...
export function renderScript(paths: { socket: string; spool: string }): string {
  ...
  return [
    ...
    'at=$(($(date +%s) * 1000))',
    '',
    "# tmux's pane-died hook passes the dead pane's exit status and the name of",
    '# the signal that killed it; Claude passes neither. tmux fills in exactly',
    '# one of the two — a status with no signal, or a signal with no status —',
    '# so the signal is checked first and a death reporting neither is dropped',
    '# rather than sent as a mystery the parser would only refuse.',
    '#',
    '# Both are checked against their shape before being interpolated. The',
    '# status goes in *unquoted* (a quoted one would arrive as a string and be',
    '# refused), and the signal goes inside JSON quotes it could otherwise end.',
    'if [ -n "$signal" ]; then',
    '  case "$signal" in',
    '    *[!A-Za-z0-9]*) exit 0 ;;',
    '  esac',
