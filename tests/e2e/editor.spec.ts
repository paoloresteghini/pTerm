...
/*
 * `two editor panes on one file` used to live here. It built its two panes by
 * clicking one tree row twice, which `openEditor`'s dedup no longer produces:
 * a file that is already open gets its existing pane back. The behaviour it
 * covered — a save from one pane refusing the other — moved to
 * `editorRestore.spec.ts`, which seeds the two panes instead.
 */
